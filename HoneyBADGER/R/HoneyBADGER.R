...
HoneyBADGER$methods(
    setAlleleMats=function(r.init, n.sc.init, l.init=NULL, n.bulk.init=NULL, filter=TRUE, het.deviance.threshold=0.05, min.cell=3, n.cores=1, verbose=TRUE) {
        if(verbose) {
          cat("Initializing allele matrices (local) ... \n")
        }
...
#'
HoneyBADGER$methods(
  setAlleleMatsBetaBinomial=function(r.init, n.sc.init, l.init=NULL, n.bulk.init=NULL, filter=TRUE, het.deviance.threshold=0.05, min.cell=3, n.cores=1, verbose=TRUE) {
    if(verbose) {
      cat("Initializing allele matrices (local) ... \n")
    }
    ...
    # Recenter the observed allele fractions around 0.5
    if(verbose) {
      cat("Recentering observed allele fractions around 0.5 ... \n")
    }
    
    # Calculate the sum of r.maf and n.sc for each SNP across all cells
    total_maf_sum <- rowSums(r.maf)
    total_sc_sum <- rowSums(n.sc)
    
    # Calculate the frequency of the minor allele for each SNP across all cells
    maf_frequency <- total_maf_sum / total_sc_sum
    
    # Calculate the normalization factor to bring the frequency to 0.5
    normalization_factor <- 0.5 / maf_frequency
    
    # Normalize the allele fractions by multiplying r.maf by the normalization factor for each SNP
    normalized_r_maf <- sweep(r.maf, 1, ((normalization_factor+1)/2), FUN="*")
    
    # Round the normalized_r_maf to the nearest integer while keeping the values within the bounds (0 < r.maf < n.sc)
    rounded_normalized_r_maf <- pmin(pmax(round(normalized_r_maf), 0), pmax(n.sc - 1, 0))

    #print("old rmaf")
    #print(head(r.maf))
    #print("normalization factor")
    #print(head(normalization_factor))
    #print("normalized_r_maf")
    #print(head(normalized_r_maf))
    #print("new rmaf")
    #print(head(rounded_normalized_r_maf))
    
    # Update the r.maf matrix with the rounded_normalized_r_maf
    r.maf <<- rounded_normalized_r_maf
    
    if(verbose) {
      cat("Done normalizing observed allele fractions to 0.5 on a per-SNP basis! \n")
    }
...
HoneyBADGER$methods(
    calcAlleleCnvBoundaries=function(r.sub=NULL, n.sc.sub=NULL, l.sub=NULL, n.bulk.sub=NULL, min.traverse=3, t=1e-6, pd=0.1, pn=0.45, min.num.snps=5, trim=0.1, init=FALSE, verbose=FALSE, ...) {
        ...
        # uncomment above line and delete below to revert PCA change. 
        #print("attempting dimensionality reduction - Ahmed")
        #library(stats)
        # Perform PCA
        #mat.tot[is.nan(mat.tot) | is.infinite(mat.tot)] <- 0
        #pca_res <- prcomp(mat.tot, center = TRUE, scale. = TRUE)
        # Decide the number of principal components to retain (optional)
        # You can set a custom number or use a proportion of variance explained as a threshold
        #var_explained <- (pca_res$sdev^2 / sum(pca_res$sdev^2))
        #cum_var_explained <- cumsum(var_explained)
        #num_pcs_to_retain <- min(which(cum_var_explained >= 0.95)) # Retain components that explain 95% of the variance
        # Retain only the selected principal components
        #reduced_data <- pca_res$x[, 1:num_pcs_to_retain]
        #mat.smooth <- apply(reduced_data, 2, caTools::runmean, k=31)
...
#'
HoneyBADGER$methods(
  calcAlleleCnvBoundariesPCA=function(r.sub=NULL, n.sc.sub=NULL, l.sub=NULL, n.bulk.sub=NULL, min.traverse=3, t=1e-6, pd=0.1, pn=0.45, min.num.snps=5, trim=0.1, init=FALSE, verbose=FALSE, ...) {
    ...
    ## normalize the frequency of the lesser allele to 0.5 (EXPERIMENTAL)
    # Calculate the sum of alternative alleles and total alleles for each row, ignoring NaN values
    alt_allele_sum <- apply(mat.tot, 1, function(x) sum(x, na.rm = TRUE))
    total_allele_sum <- apply(!is.nan(mat.tot), 1, function(x) sum(x))
    # Update the prior with the observed data to get the posterior parameters
    posterior_alpha <- 1 + alt_allele_sum
    posterior_beta <- 1 + total_allele_sum - alt_allele_sum
    # Calculate the expected allele frequency given the posterior parameters
    expected_allele_freq <- posterior_alpha / (posterior_alpha + posterior_beta)
    # Calculate the normalization factor for each allele
    norm_factor <- 0.5 / expected_allele_freq
    # Apply the normalization factor to each allele, preserving NaN values
    normalized_mat <- mat.tot * outer(norm_factor, rep(1, ncol(mat.tot)))
    # Ensure that the original NaN values are preserved in the normalized matrix
    normalized_mat[is.nan(mat.tot)] <- NaN
    
    #print("old")
    #print(mat.tot)
    mat.tot <- normalized_mat
    #print("New")
    #print(mat.tot)
...
#'
HoneyBADGER$methods(
  calcAlleleCnvBoundariesSimple=function(r.sub=NULL, n.sc.sub=NULL, l.sub=NULL, n.bulk.sub=NULL, min.traverse=3, t=1e-6, pd=0.1, pn=0.45, min.num.snps=5, trim=0.1, init=FALSE, verbose=FALSE, ...) {
...
#'
HoneyBADGER$methods(
  calcAlleleCnvBoundariesDBSCAN=function(r.sub=NULL, n.sc.sub=NULL, l.sub=NULL, n.bulk.sub=NULL, min.traverse=3, t=1e-6, pd=0.1, pn=0.45, min.num.snps=5, trim=0.1, init=FALSE, verbose=FALSE, ...) {
    ...
    #mat.smooth <- apply(mat.tot, 2, caTools::runmean, k=31)
    # uncomment above line and delete below to revert DBSCAN change. 
    #print("attempting dimensionality reduction via DBSCAN - Ahmed")
    # Load required libraries
    library(dbscan)
    library(stats)
    # Handle NaN and infinite values in the mat.tot matrix
    mat.tot[is.nan(mat.tot) | is.infinite(mat.tot)] <- 0
    # Run DBSCAN for clustering
    eps_value <- 0.5 # Adjust the epsilon value based on the dataset
    minPts_value <- 5 # Adjust the minimum number of points in the neighborhood based on the dataset
    dbscan_result <- dbscan::dbscan(mat.tot, eps = eps_value, minPts = minPts_value)
...
#'
HoneyBADGER$methods(
  calcAlleleCnvBoundariesOld=function(r.sub=NULL, n.sc.sub=NULL, l.sub=NULL, n.bulk.sub=NULL, min.traverse=3, t=1e-6, pd=0.1, pn=0.45, min.num.snps=5, trim=0.1, init=FALSE, verbose=FALSE, ...) {
